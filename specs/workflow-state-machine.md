...
| Collect maintainers | Run `scripts/get_maintainer.pl` for the touched files and capture recipients. Write a `git send-email patch/... --to=... --cc=...` command into `${kernel_tree}/tools/testing/report2patch/${work_branch}/commit.md`, using one primary `--to=` recipient and placing every other recipient under `--cc=`. | Move to `Assemble final bundle`. | Continue with a partial bundle if the script is unavailable, but say so explicitly. |
