...
This repository packages one Codex skill plus supporting reference material. The skill itself is documentation-driven: behavior is defined by prompt text in `SKILL.md`, narrowed by `references/`, and sanity-checked by the local unit tests.
...
- Canonical prompt logic lives in `SKILL.md` plus the Markdown files under `references/`.
...
4. Blocking environment/build/runtime invocation problems are written to `${kasan_artifact_dir}/failure-notes.md`.
5. Patch artifacts are written under `patch_output_dir`.
6. Final status and maintainer routing are assembled from the state machine and output-bundle contract.
